            ...
            <div className="glass-card w-full max-w-md p-8 relative overflow-hidden">
                ...
                {step === 1 && (
                    <div className="animate-fade-in space-y-6">
                        <div className="flex items-center space-x-3 text-[var(--color-accent-blue)] mb-4">
                            <User size={24} />
                            <h2 className="text-2xl font-bold text-[var(--color-text-primary)]">Subject Identity</h2>
                        </div>
                        <p className="text-[var(--color-text-secondary)]">Enter designation for protocol tracking.</p>
                        <input
                            type="text"
                            name="name"
                ...
                {step === 2 && (
                    <div className="animate-fade-in space-y-6">
                        <div className="flex items-center space-x-3 text-[var(--color-accent-blue)] mb-4">
                            <Calculator size={24} />
                            <h2 className="text-2xl font-bold text-[var(--color-text-primary)]">Chronological Status</h2>
                        </div>
                        <p className="text-[var(--color-text-secondary)]">Current age is required for baseline calibration.</p>
                        <input
                            type="number"
                            name="age"
                ...
                {step === 3 && (
                    <div className="animate-fade-in space-y-6">
                        <div className="flex items-center space-x-3 text-[var(--color-accent-blue)] mb-4">
                            <Target size={24} />
                            <h2 className="text-2xl font-bold text-[var(--color-text-primary)]">Priority Vector</h2>
                        </div>
                        <p className="text-[var(--color-text-secondary)]">Select primary aging vector to target immediately.</p>
                        <div className="grid grid-cols-1 gap-3">
                            {['Physical', 'Mental', 'Emotional', 'Social', 'Intellectual'].map((area) => (
                                <button
                                    key={area}
                                    onClick={() => setFormData({ ...formData, focus: area.toLowerCase() })}
                                    className={`p-3 rounded border text-left transition-all ${formData.focus === area.toLowerCase()

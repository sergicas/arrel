...
const Social = () => {
  const { saveData } = useArrelData();

  // ---- STATE: Connections Check ----
  // ---- STATE: Connections Check ----
  const [connections, setConnections] = useState(() => {
    // Lazy load from storage via loadData if possible, or defaulting
    // Since loadData might be context-based, we can try to use it if stable,
    // but usually Context + lazy init is tricky if loadData isn't pure.
    // However, the error suggests loadData is synchronous.
    // Let's assume we can read directly or we just keep the effect but check for difference?
    // "Calling setState synchronously within an effect" happens because loadData returns immediately.
    // Better pattern:
    try {
      const parsed = JSON.parse(localStorage.getItem('arrel_social')); // Direct access if we know the key strategy
  ...
  const [socialGoal, setSocialGoal] = useState(() => {
    try {
      const parsed = JSON.parse(localStorage.getItem('arrel_social'));
      return parsed?.socialGoal || '';
    } catch { return ''; }
  });

  // Removed useEffect for loading as we use lazy init
      ...
      <div className="max-w-md mx-auto space-y-8">
        ...
        <div className="bg-white/70 backdrop-blur-xl border border-white/50 rounded-3xl p-6 shadow-xl space-y-6">
          ...
          <p className="text-sm text-gray-500 mb-4">
            Identifica 3 persones clau per al teu benestar i quan vas parlar amb elles per últim
            cop.
          </p>
          ...
          <div className="space-y-4">
            {connections.map((conn) => (
              <div
                key={conn.id}
                className="bg-white/60 p-4 rounded-xl border border-blue-50 flex flex-col gap-3"
              >
                ...
                <div className="flex items-center gap-2 text-xs text-gray-500 pl-10">
                  <span className="uppercase tracking-wider font-bold text-[10px] text-gray-400">
                    Últim contacte:
                  </span>
                  <input
                    type="text"
        ...
        <div className="bg-gradient-to-br from-blue-500 to-indigo-600 rounded-3xl p-8 shadow-xl text-white relative overflow-hidden">
          <div className="relative z-10">
            ...
            <p className="text-xs opacity-80 leading-relaxed">
              Una conversa real de 10 minuts redueix cortisol i augmenta oxitocina. Els missatges de
              text no tenen el mateix efecte biològic.
            </p>
          </div>

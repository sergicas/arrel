...
export default function DiaCheckIn() {
    ...
    const dailyContent = {
        2: { theme: "Consistència", tip: "Avui és el primer dia real. No busquis perfecció, busca completar." },
        3: { theme: "Ajustament", tip: "Si ahir va costar, ajusta la intensitat. L'important és no parar." },
        4: { theme: "Resistència", tip: "És normal sentir resistència. El teu cervell prefereix la comoditat anterior." },
        5: { theme: "Observació", tip: "Fixa't en com et sents després de complir. Aquesta és la teva gasolina." },
        6: { theme: "Optimització", tip: "Pots fer els protocols més fàcils? Prepara't la nit abans." },
        7: { theme: "Celebració", tip: "Has completat una setmana. Això és més que la majoria. Continua." },
    };

    const currentContent = dailyContent[dayNumber] || { theme: "Avui", tip: "Cada dia és una nova oportunitat per millorar la teva biologia." };

    // Estat dels protocols (Càrrega dinàmica)
    const [protocols, setProtocols] = useState([]);

    useEffect(() => {
        // Carregar protocols personalitzats si existeixen
        const resultatsDia1 = localStorage.getItem('arrel_resultats_dia1');

        if (resultatsDia1) {
            try {
                const { protocol } = JSON.parse(resultatsDia1);
                // Mapejar al format de DiaCheckIn
                const personalizedProtocols = protocol.map((acc, idx) => ({
                    id: `p_${idx}`,
                    title: acc.titol,
                    icon: idx === 0 ? Moon : (idx === 1 ? Coffee : Sun), // Assignació simple d'icones per defecte
                    status: 'pending',
                    color: idx === 0 ? 'indigo' : (idx === 1 ? 'orange' : 'teal')
                }));
                setProtocols(personalizedProtocols);
            } catch (e) {
    ...
    // Si estem editant un dia passat, carregar les dades existents
    useEffect(() => {
        const progresGuardat = localStorage.getItem('arrel_progres');
        if (progresGuardat) {
            const data = JSON.parse(progresGuardat);
            const entry = data.find(d => d.dia === dayNumber);

            if (entry && entry.protocols) {
                // Restaurar estat protocols
                setProtocols(prev => prev.map(p => {
                    const savedP = entry.protocols.find(sp => sp.name === p.id);
                    return savedP ? { ...p, status: savedP.status } : p;
                }));
                // Restaurar sliders si existeixen (assumint que guardarem això en el futur)
                if (entry.energia) setEnergia(entry.energia);
                if (entry.anim) setAnim(entry.anim);
            }
        }
    }, [dayNumber]);
                ...
                <div className="max-w-md mx-auto">
                    ...
                    <div className="flex justify-between items-end">
                        <div>
                            ...
                            <p className="text-purple-600 font-medium">{currentContent.theme}: <span className="text-gray-600 font-normal">{currentContent.tip}</span></p>
                        </div>
                        {/* Circle Progress */}

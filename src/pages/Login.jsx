...
const Login = () => {
  ...
  const { signIn, signUp, enterAsGuest, resetPassword } = useAuth();
  ...
  const handleForgotPassword = async (e) => {
    ...
    if (error) {
      setMessage({ type: 'error', text: 'Error en enviar el correu. Comprova l\'adreça.' });
    } else {
      setMessage({ type: 'success', text: 'Correu enviat! Revisa la safata d\'entrada per restablir la contrasenya.' });
    }
    setLoading(false);
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-purple-50 to-pink-50 text-gray-900 flex flex-col items-center justify-center p-6 animate-enter">
      <SEO
        title={mode === 'login' ? 'Accés Usuaris' : mode === 'register' ? 'Crear Compte' : 'Restablir Contrasenya'}
      ...
      <div className="w-full max-w-sm bg-white/80 backdrop-blur-xl p-8 rounded-2xl shadow-xl border border-white/50">
        ...
        <div className="flex gap-4 mb-8 border-b border-gray-100 pb-2">
          {mode === 'forgot' ? (
            <button
              ...
              className="pb-2 text-sm font-bold text-gray-400 hover:text-gray-600 transition-all flex items-center gap-1"
            >
              ← Tornar
            </button>
          ) : (
        ...
        <div className="mb-8 text-center">
          <h1 className="text-2xl font-bold tracking-tight text-gray-900 mb-2">
            {mode === 'login' ? 'Benvingut de nou' : mode === 'register' ? 'Comença el teu viatge' : 'Restablir contrasenya'}
          </h1>
        ...
        <form onSubmit={mode === 'forgot' ? handleForgotPassword : handleSubmit} className="space-y-4">
          ...
          {mode === 'login' && (
            <div className="text-right -mt-2">
              <button
                type="button"
                onClick={() => { setMode('forgot'); setMessage(null); setPassword(''); }}

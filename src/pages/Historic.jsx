...
import { LineChart, ArrowRight, Activity, Filter, Calendar, ChevronDown, ChevronUp, FileDown } from 'lucide-react';
import SEO from '../components/SEO';

export default function Historic() {
    const navigate = useNavigate();
    const [historic, setHistoric] = useState([]);
    const [diagnosticsAmbPuntuacions, setDiagnosticsAmbPuntuacions] = useState([]);
    const [filtre, setFiltre] = useState('global'); // 'global', 'energia', 'son', 'nutricio', 'atencio', 'temps'
    const [nextProtocolDay, setNextProtocolDay] = useState(2);

    useEffect(() => {
        // Carregar històric
        const historicGuardat = localStorage.getItem('arrel_historic');
        const historicData = historicGuardat ? JSON.parse(historicGuardat) : [];
        setHistoric(historicData);

        // Carregar progrés protocols
        const progresGuardat = localStorage.getItem('arrel_progres');
        const progresData = progresGuardat ? JSON.parse(progresGuardat) : [];

        // Calcular proper dia
        const maxDayCompleted = progresData.reduce((max, entry) => Math.max(max, entry.dia || 0), 1);
        setNextProtocolDay(maxDayCompleted + 1);

        console.log("Dades Progrés carregades:", progresData);

        // Calcular puntuacions per cada diagnòstic
        const ambPuntuacions = historicData.map(diag => {
            ...
            const relatedProtocols = progresData.filter(p => {
        ...
        setDiagnosticsAmbPuntuacions(ambPuntuacions);
    }, []);

    function calcularPuntuacions(respostes) {
        // Assegurar que tenim prou respostes (omplir amb 0 si falta alguna) per evitar errors
        const r = respostes.length >= 17 ? respostes : [...respostes, ...Array(17 - respostes.length).fill(0)];
    ...
    const SimpleGraph = ({ data, dataKey }) => {
        ...
        return (
            <div className="w-full h-32 bg-white/50 rounded-xl p-4 mb-8 shadow-sm border border-white/50 backdrop-blur-sm relative overflow-hidden">
                <div className="absolute top-2 left-4 text-xs font-bold text-gray-500 uppercase tracking-widest">Tendència: {dataKey.toUpperCase()}</div>
                <svg viewBox="0 0 100 100" preserveAspectRatio="none" className="w-full h-full pt-4">
                    ...
                    <line x1="0" y1="80" x2="100" y2="80" stroke="#e5e7eb" strokeWidth="0.5" strokeDasharray="2" />
...
function HistoricCard({ diag, idx, filtre, navigate }) {
    ...
    const StatusText = ({ status }) => {
        if (status === 'completed') return <span className="text-green-700 bg-green-50 px-2 py-0.5 rounded text-xs font-medium">Fet</span>;
        if (status === 'partial') return <span className="text-orange-700 bg-orange-50 px-2 py-0.5 rounded text-xs font-medium">Parcial</span>;
        return <span className="text-gray-500 bg-gray-50 px-2 py-0.5 rounded text-xs font-medium">No fet</span>;
    };
    ...
    return (
        <div className="bg-gradient-to-br from-white to-gray-50 rounded-2xl shadow-lg hover:shadow-xl transition-shadow p-6 border border-gray-100 relative overflow-hidden group">
            ...
            <div className={`absolute top-0 right-0 w-24 h-24 -mr-8 -mt-8 rotate-45 flex items-end justify-center pb-2 shadow-sm ${getBgScore(diag.puntuacions.global)}`}>
                <span className="font-bold text-lg transform -rotate-45 translate-y-1">{diag.puntuacions.global}</span>
            </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-3">
                ...
                <div className={`p-3 rounded-xl bg-white border shadow-sm ${filtre === 'global' ? 'ring-2 ring-purple-400 border-purple-400' : 'border-gray-100'} transition-all`}>
                    <div className={`text-2xl font-bold mb-1 ${getColorScore(diag.puntuacions.global)}`}>
                        {diag.puntuacions.global}
                    </div>
                    <div className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Global</div>
                </div>

                {/* ENERGIA */}
                <div className={`p-3 rounded-xl bg-white border shadow-sm ${filtre === 'energia' ? 'ring-2 ring-orange-400 border-orange-400' : 'border-gray-100'} transition-all`}>
                    <div className="text-2xl font-bold text-orange-500 mb-1">
                        {diag.puntuacions.energia}
                    </div>
                    <div className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Energia</div>
                </div>

                {/* SON */}
                <div className={`p-3 rounded-xl bg-white border shadow-sm ${filtre === 'son' ? 'ring-2 ring-indigo-400 border-indigo-400' : 'border-gray-100'} transition-all`}>
                ...
                {/* NUTRICIÓ */}
                <div className={`p-3 rounded-xl bg-white border shadow-sm ${filtre === 'nutricio' ? 'ring-2 ring-green-400 border-green-400' : 'border-gray-100'} transition-all`}>
                    <div className="text-2xl font-bold text-green-500 mb-1">
                        {diag.puntuacions.nutricio}
                    </div>
                    <div className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Nutrició</div>
                </div>

                {/* ATENCIÓ */}
                <div className={`p-3 rounded-xl bg-white border shadow-sm ${filtre === 'atencio' ? 'ring-2 ring-blue-400 border-blue-400' : 'border-gray-100'} transition-all`}>
                    <div className="text-2xl font-bold text-blue-500 mb-1">
                        {diag.puntuacions.atencio}
                    </div>
                    <div className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Atenció</div>
                </div>

                {/* TEMPS */}
                <div className={`p-3 rounded-xl bg-white border shadow-sm ${filtre === 'temps' ? 'ring-2 ring-purple-400 border-purple-400' : 'border-gray-100'} transition-all`}>
            ...
            <div className="mt-4 pt-4 border-t border-gray-200">
                ...
                >
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <div className="flex gap-2 flex-wrap justify-end max-w-[200px] sm:max-w-none">
                            {diag.protocols && diag.protocols.map((p, i) => (
                                <div key={i} className={`flex items-center gap-1 text-xs px-2 py-1 rounded ${p.compliment === 100 ? 'bg-green-50 text-green-700' : 'bg-gray-100 text-gray-600'}`}>
                                    <span>Dia {p.dia}:</span>
                        ...
                        {/* Icona expandir */}
                        <div className={`p-1 rounded-full bg-gray-100 group-hover/btn:bg-gray-200 transition-all duration-300 ${expanded ? 'bg-purple-100 text-purple-600 rotate-180' : 'text-gray-400'}`}>
                            <ChevronDown size={16} />
                        </div>
                    </div>
                </button>
                ...
                {/* Detall Expandible */}
                <div className={`overflow-hidden transition-all duration-300 ease-in-out ${expanded ? 'max-h-[500px] opacity-100 mt-4 overflow-y-auto' : 'max-h-0 opacity-0'}`}>
                    <div className="bg-gray-50 rounded-xl p-4 space-y-4">
                        {diag.protocols && diag.protocols.length > 0 ? (
                                    ...
                                    <div className="space-y-2">
                                        {protocol.protocols ? (
                                            protocol.protocols.map((p, idx) => (
                                                <div key={idx} className="flex items-center justify-between text-sm">
                                                    <div className="flex items-center gap-2">
                                                        <span className="text-base"><StatusIcon status={p.status} /></span>
                                                        <span className="text-gray-700 font-medium">{getProtocolName(p.name)}</span>
                                                    </div>
                                        ...
                                        <div className="flex gap-4 mt-2 pt-2 border-t border-dashed border-gray-100">
                                            <div className="text-xs text-gray-500">
                                                Energia: <span className="font-bold text-gray-700">{protocol.energia || '-'}</span>
                                            </div>
                                            <div className="text-xs text-gray-500">
                                                Ànim: <span className="font-bold text-gray-700">{protocol.anim || '-'}</span>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div className="text-center py-4">
                                <p className="text-sm text-gray-500 mb-2">No hi ha dades de protocols per aquest període.</p>
                            </div>
                        )}
                    </div>
                </div>

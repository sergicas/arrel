...
const Mental = () => {
    ...
    // ---- STATE: Gratitude ----
    const initialGratitude = ['', '', ''];
    const [gratitudeLines, setGratitudeLines] = useState(initialGratitude);

    // ---- LOAD STATE ----
    useEffect(() => {
        const parsed = loadData('mental');
        if (parsed && parsed.gratitude) {
            setGratitudeLines(parsed.gratitude);
        }
    }, [loadData]);
        ...
        if (breathingActive) {
            interval = setInterval(() => {
                setTimeLeft((prev) => {
                    ...
                    // Transition phase
                    switch (phase) {
                        case 'idle': return 4; // Should not happen if active
                        case 'inhale': setPhase('hold_in'); return 4;
                        case 'hold_in': setPhase('exhale'); return 4;
                        case 'exhale': setPhase('hold_out'); return 4;
                        case 'hold_out': setPhase('inhale'); return 4;
                        default: return 4;
                    }
                });
            }, 1000);
    ...
    const getPhaseText = () => {
        switch (phase) {
            case 'idle': return 'Prem per començar';
            case 'inhale': return 'Inspira...';
            case 'hold_in': return 'Mantingues...';
            case 'exhale': return 'Expira...';
            case 'hold_out': return 'Buit...';
            default: return '';
        }
    };
    ...
    const getCircleScale = () => {
        ...
        switch (phase) {
            case 'inhale': return 'scale-100 duration-[4000ms]';
            case 'hold_in': return 'scale-100 duration-0';
            case 'exhale': return 'scale-50 duration-[4000ms]';
            case 'hold_out': return 'scale-50 duration-0';
            ...
            <div className="max-w-md mx-auto space-y-8">
                ...
                <div className="bg-white/70 backdrop-blur-xl border border-white/50 rounded-3xl p-8 shadow-xl relative overflow-hidden flex flex-col items-center justify-center min-h-[350px]">
                    ...
                    <div className="relative flex items-center justify-center mb-8 mt-4">
                        {/* Outer Glow Ring */}
                        <div className={`absolute rounded-full border-4 border-indigo-100 opacity-50 transition-all ease-linear
                            ${phase === 'inhale' ? 'w-64 h-64 border-indigo-200' :
                                phase === 'exhale' ? 'w-32 h-32' :
                                    phase === 'hold_in' ? 'w-64 h-64' :
                                        phase === 'hold_out' ? 'w-32 h-32' : 'w-48 h-48'}
                        `}></div>
                        ...
                        <div
                            ...
                            ${phase === 'inhale' ? 'w-64 h-64 duration-[4000ms]' :
                                    phase === 'exhale' ? 'w-32 h-32 duration-[4000ms]' :
                                        phase === 'hold_in' ? 'w-64 h-64 duration-100' :
                                            phase === 'hold_out' ? 'w-32 h-32 duration-100' : 'w-40 h-40 duration-500'}
                            `}
                        >
                ...
                <div className="bg-white/70 backdrop-blur-xl border border-white/50 rounded-3xl p-6 shadow-xl space-y-4">
                    ...
                    <p className="text-sm text-gray-500 mb-4">Escriu 3 coses simples per les quals et sentis agraït avui.</p>

                    <div className="space-y-3">
                        {gratitudeLines.map((line, idx) => (
                            <div key={idx} className="relative">
                                <span className="absolute left-3 top-3 text-gray-400 font-serif italic text-sm">{idx + 1}.</span>
                                <input
                                    type="text"
                                    value={line}

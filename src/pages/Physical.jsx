...
const TASK_CONFIG = [
  ...
  {
    id: 'nutrition',
    label: 'Menjar Real (NO processats)',
    icon: Monitor,
...
const Physical = () => {
  ...
  const [tasks, setTasks] = useState(() => {
    try {
      const parsed = JSON.parse(localStorage.getItem('arrel_physical'));
      if (parsed) {
        return TASK_CONFIG.map((task) => {
  ...
  // Progress effect is handled above
          ...
          {tasks.map((task) => (
            ...
            >
              <div className="flex items-center gap-4 z-10">
                <div
                  className={`p-3 rounded-xl ${task.completed ? task.bg : 'bg-gray-100'} transition-colors duration-300`}
                >
                  <task.icon size={20} className={task.completed ? task.color : 'text-gray-400'} />
                </div>
                <span
                  className={`font-medium transition-colors ${task.completed ? 'text-gray-900' : 'text-gray-500'}`}
                >
                  {task.label}
                </span>
              </div>

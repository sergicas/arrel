      ...
      <div className="relative bg-white rounded-3xl w-full max-w-2xl max-h-[90vh] overflow-y-auto shadow-2xl animate-in zoom-in-95 duration-200">
        ...
        <div className="h-32 bg-gray-900 overflow-hidden relative">
          ...
          <button
            ...
            className={`absolute top-4 right-16 rounded-full p-2 transition-colors z-10 ${isBookmarked
        ...
        <div className="p-6 md:p-8 -mt-12 relative">
          ...
          <div className="inline-flex items-center justify-center w-20 h-20 bg-white rounded-2xl shadow-lg text-purple-600 mb-6 relative">
            ...
            {isRead && (
              <div
                className="absolute -top-2 -right-2 bg-green-500 text-white rounded-full p-1 border-2 border-white"
                title="Llegit"
              >
          ...
          <div className="flex flex-wrap items-center gap-2 mb-2">
            ...
            <span
              className={`text-[10px] font-bold px-3 py-1 rounded-full uppercase tracking-wider border ${resource.difficulty === 'bàsic'
                ? 'bg-green-50 text-green-700 border-green-100'
                : resource.difficulty === 'intermedi'
          ...
          <p className="text-lg text-gray-600 mb-8 leading-relaxed">{resource.summary}</p>
          ...
          <div className="flex gap-4">
            <a
              ...
              className="flex-1 bg-gray-900 text-white font-bold py-4 px-6 rounded-xl flex items-center justify-center gap-2 hover:bg-black transition-all hover:scale-[1.01] shadow-xl shadow-purple-900/10 whitespace-nowrap"
              onClick={() => {
                markAsRead(resource.id);
                onClose();
              }}
            >
              {resource.type === 'video' ? 'Veure Vídeo' : 'Llegir Article'}{' '}
...
export default function Recursos() {
  // const { loadData } = useArrelData(); // Unused
  // Simulate loading state locally if useArrelData doesn't provide it,
  // or just assume we want to show it on mount for effect.
  // For this "Quick Win", let's add a local loading state to mimic data fetching.
  ...
  const [bookmarks, setBookmarks] = useState(() => {
    const saved = secureStorage.getItem('arrel_bookmarks');
    return saved ? new Set(saved) : new Set();
  });
  const [readHistory, setReadHistory] = useState(() => {
    const saved = secureStorage.getItem('arrel_history');
    return saved ? new Set(saved) : new Set();
  });
  const [recommendedArea] = useState(() => {
    ...
    try {
      const scores = secureStorage.getItem('arrel_user_scores');
      if (scores) {
        areas.forEach((area) => {
          if (scores[area] < minScore) {
            minScore = scores[area];
            targetArea = area;
          }
        });
      }
      if (!targetArea && scores) targetArea = 'son';
    } catch {
      // ignore
    }
  ...
  // Removed useEffect for recommendations
  ...
  const toggleBookmark = (id) => {
    ...
    setBookmarks(newSet);
    secureStorage.setItem('arrel_bookmarks', [...newSet]);
  };
  ...
  const markAsRead = (id) => {
    ...
    secureStorage.setItem('arrel_history', [...newSet]);
  };
  ...
  const filtered = resourcesData.filter((r) => {
    const matchCategory =
      ...
      (activeTab === 'bookmarks' && bookmarks.has(r.id));
    ...
    <div className="min-h-screen bg-gray-50 p-6 md:p-12">
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="mb-12 text-center animate-enter">
          ...
          <h1 className="text-4xl md:text-5xl font-bold text-gray-900 mb-4">Recursos & Acadèmia</h1>
          <p className="text-gray-500 text-sm font-mono tracking-tight flex items-center justify-center gap-2">
            <span className="w-2 h-2 bg-green-500 rounded-full animate-pulse"></span>
            Última actualització editorial: Gener 2026
          </p>
        </div>

        {/* CURATOR'S NOTE & CRITERIA */}
        <div className="bg-white border border-gray-200 p-8 rounded-2xl shadow-sm mb-12 max-w-4xl mx-auto text-center relative overflow-hidden">
          <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-purple-500 to-blue-500"></div>
          <Star className="w-8 h-8 text-purple-600 mx-auto mb-4" />
          <h3 className="text-xl font-bold text-gray-900 mb-3 font-serif italic">
            "Evidència sobre modes passatgeres"
          </h3>
          <p className="text-gray-600 leading-relaxed max-w-2xl mx-auto mb-6">
            A internet hi ha massa soroll. Aquí només trobaràs eines i lectures que han passat el
            nostre filtre editorial:{' '}
            <strong>rigor científic, aplicabilitat real i impacte demostrable</strong>. No afegim
            res "per omplir".
          </p>
          <div className="flex flex-wrap justify-center gap-4 text-xs font-bold text-gray-400 uppercase tracking-widest">
            <span className="flex items-center gap-1">
              <Check size={14} className="text-purple-600" /> Curat per Humans
            </span>
            <span className="flex items-center gap-1">
              <Check size={14} className="text-purple-600" /> Sense Patrocinis
            </span>
            <span className="flex items-center gap-1">
              <Check size={14} className="text-purple-600" /> Revisat 2026
            </span>
          </div>
        </div>
          ...
          <div className="mb-16 bg-gradient-to-r from-indigo-50 to-purple-50 rounded-3xl p-8 border border-purple-100 relative overflow-hidden">
            ...
            <div className="flex items-center gap-3 mb-6 relative z-10">
              ...
              <h2 className="text-xl font-bold text-gray-900">
                Seleccionat per a tu:{' '}
                <span className="capitalize text-purple-600">{recommendedArea}</span>
              </h2>
            </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 mb-12 flex flex-col md:flex-row justify-between items-center gap-4 sticky top-4 z-30">
          ...
          <div className="flex overflow-x-auto pb-2 md:pb-0 gap-2 w-full md:w-auto scrollbar-hide">
            {[
              ...
              >
                {tab.icon && (
                  <tab.icon size={12} fill={activeTab === tab.id ? 'currentColor' : 'none'} />
                )}
                {tab.label}
              </button>
            ))}
              ...
              >
                ...
                <div className="absolute top-4 right-4 flex gap-2">
                  ...
                  {bookmarks.has(resource.id) && (
                    <div
                      className="bg-yellow-100 text-yellow-600 p-1.5 rounded-full"
                      title="Guardat"
                    >
                ...
                <div className="flex justify-between items-start mb-4 pr-10">
                  <div
                    className={`p-3 rounded-xl transition-colors ${resource.type === 'video'
                      ? 'bg-red-50 text-red-600'
                      : resource.category === 'suplements'
                  ...
                  >
                    {/* Link Icon fallback */}
                    {resource.icon ? (
                      React.createElement(resource.icon, { size: 24 })
                    ) : (
                      <BookOpen size={24} />
                    )}
                  </div>
                ...
                <div className="flex gap-2 mb-2">
                  ...
                  <span
                    className={`text-[10px] font-bold px-2 py-0.5 rounded uppercase tracking-wider ${resource.difficulty === 'bàsic'
        ...
        <div className="mt-24 bg-gray-900 rounded-3xl p-8 md:p-16 text-center text-white relative overflow-hidden">
          ...
          <div className="relative z-10">
            ...
            <p className="text-lg text-gray-300 max-w-2xl mx-auto mb-8">
              Estem preparant una sèrie de masterclasses en vídeo amb experts en longevitat.

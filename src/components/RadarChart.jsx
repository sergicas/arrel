...
const RadarChart = ({ data, size = 300 }) => {
    ...
    const getCoordinates = (value, index) => {
        const angle = (Math.PI / 2) + (index * angleStep); // Start from top (-90 deg effectively because SVG y is down, but let's rotate)
    ...
    return (
        <div className="relative flex justify-center items-center w-full h-auto aspect-square max-w-[350px] mx-auto">
            <svg viewBox={`0 0 ${size} ${size}`} role="img" aria-label={`Gràfic de radar: Global ${data.global}, Energia ${data.energia}, Son ${data.son}, Nutrició ${data.nutricio}, Atenció ${data.atencio}, Temps ${data.temps}`} className="w-full h-full">
                {/* Background Grid */}
                {levels.map((level) => {
            ...
            <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                <div className="flex flex-col items-center justify-center">
                    ...
                    <span className="text-[10px] text-gray-400 uppercase tracking-widest font-mono">Global</span>
                </div>
            </div>
        </div>
    );

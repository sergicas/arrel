...
const SmartTip = () => {
  const [tip] = useState(() => {
    ...
    let selectedTip = {
      ...
      text: 'No cal ser perfecte cada dia. Simplement no trenquis la cadena.',
    ...
    if (latestDiag && latestDiag.puntuacions) {
      ...
      if (scores.son < 50) {
        ...
        selectedTip = {
          ...
          text: "Si et sents dispers, el multitasking és l'enemic. Tanca totes les pestanyes menys una.",
          color: 'text-blue-600',
          bg: 'bg-blue-50',
    ...
    const dailyProgress = secureStorage.getItem('arrel_progres') || [];
  ...
  return (
    <div
      className={`mb-6 p-4 rounded-xl border ${tip.bg} ${tip.border} flex items-start gap-4 shadow-sm animate-fade-in`}
    >
      ...
      <div>
        ...
        <p className={`text-sm ${tip.color} opacity-90 leading-relaxed`}>{tip.text}</p>
      </div>
    </div>
  );
};
